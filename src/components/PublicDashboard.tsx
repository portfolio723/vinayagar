...
export default function PublicDashboard({ onAdminClick }: PublicDashboardProps) {
  ...
  return (
    <Layout>
      {/* Hero Section */}
      <div className="bg-gradient-to-br from-orange-50 via-red-50 to-pink-50 rounded-3xl p-8 lg:p-12 mb-12 border border-orange-100">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-4">
            {festivalSettings?.festival_name || 'Vinayaka Chavithi'} {festivalSettings?.festival_year || 2025}
          </h1>
          
          <p className="text-xl text-gray-700 mb-8 max-w-2xl mx-auto leading-relaxed">
            {festivalSettings?.description || 'Join us in celebrating Lord Ganesha with devotion, community spirit, and complete financial transparency.'}
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 text-gray-600">
            {festivalSettings?.location && (
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium">{festivalSettings.location}</span>
              </div>
            )}
            
            {festivalSettings?.start_date && festivalSettings?.end_date && (
              <div className="flex items-center gap-2">
                <Calendar className="w-5 h-5 text-orange-500" />
                <span className="font-medium">
                  {new Date(festivalSettings.start_date).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })} - {new Date(festivalSettings.end_date).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' })}
                </span>
              </div>
            )}
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">{financialSummary.donationCount} Contributors</span>
            </div>
          </div>
        </div>
      </div>
